...
def fenced_action(options, data, element, doc): # pylint: disable=unused-argument
    """fenced action"""
    ...
    components = []
    if title:
        modal_header1 = pf.Header(pf.Str(title),
                                  classes=['modal-title'],
                                  level=5,
                                  identifier=modalid + 'Title')
        modal_header2 = pf.Div(pf.Div(pf.Para(pf.Str('x')),
        ...
        components.append(
            pf.Div(modal_header1, modal_header2, classes=['modal-header']))
    components.append(pf.Div(*data, classes=['modal-body']))
    if closebtn:
        components.append(
            pf.Div(pf.Div(pf.Para(pf.Str('Close')),
    ...
    model_dialog = pf.Div(modal_content,
                          classes=mainclasses,
                          attributes={'role': 'document'})

    return pf.Div(model_dialog,
                  classes=['modal', 'fade'],
                  identifier=modalid,
                  attributes={
                      'tabindex': '-1',
...
def action(elem, doc):
    ...
    if isinstance(elem, pf.Link) and elem.url.startswith('modal'):
        ...
        return fenced_action(dict(id=elem.identifier, **elem.attributes),
                             elem.content, elem, doc)
    return None
