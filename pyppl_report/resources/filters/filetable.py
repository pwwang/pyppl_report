...
def format_cell(text):
    ...
    try:
        ...
        for matched in re.finditer(r'\[(.+?)\]\((.+?)\)', text):
            ...
            elems.append(
                pf.Link(pf.Str(matched.group(1)), url=matched.group(2)))
            lastend = matched.end()
        if elems:
...
def fenced_action(options, data, element, doc): # pylint: disable=unused-argument
    ...
    try:
        ...
        for i, row in enumerate(reader):
            if nrows and i > nrows:
                continue
            cells = [
                pf.TableCell(format_cell(x)) for k, x in enumerate(row)
    ...
    return pf.Div(table,
                  classes=['tablewrapper', 'datatablewrapper'],
                  attributes={'data-datatable': json.dumps(dtargs)})
